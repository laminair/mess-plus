...
class MessPlusAutomaticModelSelector:
	...
	def __init__(self, config_file_path: str, project_name: str, wandb_entity: str = None, model_family: str = "llama3"):
		...
		self.model_labels = [data['category'] for _, data in self.config["model_zoo"].items()]
	...
	def run_benchmark(
		...
		for reqtype, reqs in requests.items():
			...
			ENERGY_PER_MODEL = {i: [1e-6] for i in self.labels.keys()}  # We set an initial value to avoid zero division errors.
			...
			if args.approach != "pretrained":
				classifier = MultilabelBERTClassifier(num_labels=len(self.labels.keys()), **self.config["classifier_model"])
				classifier.make_model_if_not_exists(num_labels=len(self.labels.keys()))
	...
	def __warmup_classifier_model(self):
		...
		if self.classifier_config["use_pretrained_classifier"] is True and self.classifier_config[
			"generate_training_dataset"] is False:
